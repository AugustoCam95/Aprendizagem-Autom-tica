...
I=randperm(ColD);
Dn=Dn(:,I);
alvos=T'(:,I);   % Embaralha saidas desejadas tambem p/ manter correspondencia com vetor de entrada

% Define tamanho dos conjuntos de treinamento/teste (hold out)
ptrn=0.6    % Porcentagem usada para treino
...
for t=1:Ne,
    ...
    for tt=1:cP,   % Inicia LOOP de epocas de treinamento
        ...
        DDk = Ek;           % gradiente local (camada de saida)
...
for tt=1:cQ,
    ...
    erroTotal = [erroTotal erro*erro];
end

eqm = sum(erroTotal)/cQ
